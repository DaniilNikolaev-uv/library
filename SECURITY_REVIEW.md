...
### 1. В репозитории хранится живой `.env` для `healthcheck`
...
- Django принимает любой `Host`;
- для прод-окружения это плохая практика и лишний риск для host header атак.
...
- указать только реальные домены и нужные хосты.
...
### 4. Пароли и секреты захардкожены в `docker-compose.yml`
...
- секреты лежат прямо в compose;
- при выкладке на сервер это неудобно ротировать и легко утечь;
- в текущем виде значения слишком слабые.
...
- вынести секреты в env-файлы или secrets;
- сменить текущие значения на случайные длинные;
