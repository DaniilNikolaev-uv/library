...
- **Общий подход**:
  - **Backend**: Django + Django REST Framework, REST API, аутентификация по JWT/Token.
  - **Frontend**: Next.js (React), гибрид SPA/SSR, общение с API через fetch/axios.
  - **БД**: PostgreSQL (реляционная модель, строгие связи, транзакции).
...
- **Основные страницы**:
  ...
  - `/reader/cabinet` — личный кабинет читателя.
  - `/staff/...` — разделы для библиотекарей и администраторов.

- **Технический стек**:
  - Next.js (pages или app router).
  - React Query/SWR для работы с API.
  - UI‑библиотека (MUI/Ant Design/Tailwind + свои компоненты).
