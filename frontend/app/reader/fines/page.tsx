...
import { myFines, type Fine } from "@/lib/reader";

type Paginated<T> = { results?: T[] } & Record<string, unknown>;

export default function ReaderFinesPage() {
  const router = useRouter();
  const [data, setData] = useState<Paginated<Fine> | Fine[] | null>(null);
  const [error, setError] = useState<string | null>(null);

  const fines = useMemo(() => {
    if (!data) return [];
    if (Array.isArray(data)) return data;
    return (data.results ?? []) as Fine[];
  }, [data]);
  ...
  useEffect(() => {
    void (async () => {
      try {
        ...
        const res = await myFines();
        setData(res);
      } catch (e) {
        const err = e as ApiError;
        setError(err.detail || "Не удалось загрузить штрафы");
      }
    })();
  }, [router]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Мои штрафы</h1>
        </div>
        ...
        {fines.map((f) => (
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="font-medium">
                ...
                <span className="text-zinc-500 dark:text-zinc-400">
                  {f.inventory_number ? `• #${f.inventory_number}` : null}
                </span>
              </div>
            ...
            <div className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
              Сумма: {f.amount} • Оплачено: {f.paid_amount}
              {f.paid_at ? ` • Дата оплаты: ${f.paid_at}` : null}
            </div>
          </div>
        ))}
      </div>
